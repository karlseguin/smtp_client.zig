...
pub const Reader = struct {
	...
	// used when calling os.poll for timeouts
	pfd: [1]os.pollfd,
	...
	pub fn init(stream: Stream, timeout: i32) Reader {
		return .{
			.stream = stream,
			.timeout = timeout,
	...
	pub fn read(self: *Reader) !Reply {
		...
		while (true) {
			...
			if (comptime @TypeOf(stream) == std.net.Stream) {
				if (try std.os.poll(&self.pfd, timeout) == 0) {
...
test "reader: buffered" {
	{
		// empty
		var reader = Reader{.pfd = undefined, .stream = undefined};
...

	{
		...
		reader.pos = 5;
		try expectReply(try reader.read(0), 253, false, "");
...

	{
		...
		reader.pos = 15;
		try expectReply(try reader.read(0), 500, false, "Go Away");
...

	{
		...
		reader.pos = 21;
		try expectReply(try reader.read(0), 100, true, "");
		try expectReply(try reader.read(0), 100, true, "Hello");
...

	{
		...
		reader.pos = 22;
		try expectReply(try reader.read(0), 101, true, "");
		try expectReply(try reader.read(0), 101, true, "He1lo");
		try expectReply(try reader.read(0), 101, false, "");
		try t.expectEqual(0, reader.pos);
		try t.expectEqual(0, reader.start);
	}
}
...
test "reader: read fuzz" {
	...
	for (0..100) |_| {
		...
		var reader = Reader{.pfd = undefined, .stream = stream};
		try expectReply(try reader.read(0), 100, false, "");
		try expectReply(try reader.read(0), 101, false, "a");
		try expectReply(try reader.read(0), 200, false, "this is a bit longer");
		try expectReply(try reader.read(0), 300, true, "");
		try expectReply(try reader.read(0), 300, false, "");
		try expectReply(try reader.read(0), 301, true, "hello");
		try expectReply(try reader.read(0), 301, true, "");
		try expectReply(try reader.read(0), 301, true, "even more data");
		try expectReply(try reader.read(0), 301, false, "");
	}
}
