...
const AuthMode = lib.AuthMode;

pub fn sendToStream(stream: Stream, _: Message, config: Config) !void {
	var conn = try Client.init(stream, config);
	defer conn.deinit();
	try conn.hello();
}
...
pub const Client = struct {
	...
	pub fn init(stream: Stream, config: Config) !Client {
		var client = Client{
			.stream = stream,
			.config = config,
			.reader = Reader.init(stream, config.timeout),
		};
	...
	pub fn from(self: *Client, frm: []const u8) !void {
		var buf = &self.buf;
	...
	pub fn to(self: *Client, recepients: []const []const u8) !void {
		var buf = &self.buf;
		var reader = &self.reader;
	...
	pub fn data(self: *Client, d: []const u8) !void {
		var reader = &self.reader;
		const stream = self.stream;
	...
	fn authCRAMMD5(self: *Client) !void {
		const config = &self.config;
